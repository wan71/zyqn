module Bic_top (
    ...
    // 输出数据流接口 (M_AXIS_S2MM)
    (*mark_debug="true"*)output wire [31:0]m_axis_tdata,    // 输出数据
...
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData02             ;
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData03             ;
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData10             ;
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData11             ;
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData12             ;
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0]	             readData13             ;
...
(*mark_debug="true"*)wire [COEFF_WIDTH-1:0]	xBlend = {1'b0, xScaleAmount[SCALE_FRAC_BITS-1:SCALE_FRAC_BITS-FRACTION_BITS]};	  /* 系数*/
...
//assign o_vout_vs=i_vid_vs; 
 wire							                 i_vout_read=1;
 wire [OUTPUT_X_RES_WIDTH-1:0]	             i_des_image_x=1024-1;
...
(*mark_debug="true"*)wire [DATA_WIDTH*CHANNELS-1:0] out;
liner #(
...
cubic_interpolation #(
    .PIXEL_SIZE (8),       // 每个通道的位宽（默认8位）
    .FRACTION_BITS(8),    // 小数部分位宽（例如8位）
    .COEFF_WIDTH (9) // 插值系数的位宽（FRACTION_BITS + 符号位）
)cubic_interpolation1(
    .clk(clk),
    .out_y1(out_y1),       // 线性插值结果1（R[23:16], G[15:8], B[7:0]）
    .out_y2(out_y2),       // 线性插值结果2
    .out_y3(out_y3),       // 线性插值结果3
    .out_y4(out_y4),       // 线性插值结果4
    .xBlend(xBlend_valid), // 插值系数（9位，后8位有效）
...
////Format Q1.FRACTION_BITS
////			   yx
//parameter LIUSHUI_NUM = 26; /*用于表述流水延时*/
//parameter XBLEND_NUM = 20;
(*mark_debug="true"*)wire               liushui_valid;
(*mark_debug="true"*)wire  [8:0]             xBlend_valid;
      // 延迟实例化
    delay #(
        .DATA_WIDTH(1),
        .DELAY_CYCLES(8)
